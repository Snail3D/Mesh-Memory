#!/usr/bin/env bash
# Starter script for MESH-AI: activates venv, starts the app, and optionally opens the dashboard
set -euo pipefail
...
PID_FILE="$SCRIPT_DIR/mesh-ai.pid"
...
if [ -f "$LOCK_FILE" ]; then
  ...
  if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
    echo "ERROR: mesh-ai is already running (PID: $LOCK_PID)"
...
elif [ -f "$SCRIPT_DIR/mesh-ai/bin/activate" ]; then
  # shellcheck disable=SC1091
  . "$SCRIPT_DIR/mesh-ai/bin/activate"
...
LOG_FILE="$SCRIPT_DIR/mesh-ai.log"
PID_FILE="$SCRIPT_DIR/mesh-ai.pid"
touch "$LOG_FILE"
...
if [ -f "$SCRIPT_DIR/config.json" ]; then
  ...
  if [ -n "$cfg_port" ] && [ "$cfg_port" != "null" ]; then
    export MESH_AI_PORT="$cfg_port"
  fi
fi

if [ "${FOREGROUND:-0}" = "1" ]; then
  # Run in foreground for systemd; skip browser
  echo "Starting mesh-ai (foreground)"
  export NO_BROWSER=1
  # Create lock file with current process PID
  echo $$ > "$LOCK_FILE"
  # Exec so systemd tracks this process
  exec python "$SCRIPT_DIR/mesh-ai.py"
else
  nohup python "$SCRIPT_DIR/mesh-ai.py" >> "$LOG_FILE" 2>&1 &
  PID=$!
  echo "$PID" > "$PID_FILE"
  # Update lock file with actual Python process PID
  echo "$PID" > "$LOCK_FILE"
  echo "Started mesh-ai (pid=$PID), log=$LOG_FILE"
fi
...
if [ "${FOREGROUND:-0}" != "1" ] && [ "${NO_BROWSER:-0}" != "1" ]; then
  # Prefer explicit port exported via MESH_AI_PORT (from config.json or env)
  port=${MESH_AI_PORT:-}
  if [ -z "$port" ]; then
